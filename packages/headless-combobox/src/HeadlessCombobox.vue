...
import { computed, nextTick, onMounted, onUnmounted, ref, toRaw, useId, watch } from 'vue';
...
// A11y Prop Bindings to spread onto slots
...
function select(option: T) {
  ...
  if (props.multiple) {
    ...
    if (has) {
      emit('update:modelValue', current.filter((o) => toRaw(o) !== raw));
...
function handleClickOutside(e: MouseEvent) {
  if (isOpen.value && !isInsideWidget(e.target as Node)) {
    close(false);
  }
}
